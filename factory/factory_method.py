...
class HTTPConnector(Connector):
  ...
  def parse(self, content):
    ...
    for link in links:
      filenames.append(str(link["href"]))
...
def get_http(domain, path):
  ...
  protocol = input('Connecting to {}. Which Protocol to use? (0-http) : '.format(domain))
  ...
  try:
    ...
    parsed_content = connector.parse(content)
    servers.append(parsed_content)
    print parsed_content

  return servers

def get_ftp(domain, path):
  protocol = input('Connecting to {}. Which Protocol to use? (1-ftp): '.format(domain))
  ...
  try:
    ...
    servers.append(parsed_content[0])
    print parsed_content
  return servers
